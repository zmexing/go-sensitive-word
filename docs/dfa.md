...
### DFA 算法敏感词匹配流程动画版
文本扫描为 `"台海总统是国家领导人"`。DFA 没有 fail 指针，匹配失败需要回到 root 重新扫描。
```mermaid
flowchart TD
    %% 文本扫描流程
    ...
    T3 -->|匹配 '统', output| 统
    统 -->|记录匹配: '台海总统'| 统
    T4 -->|匹配失败 '是', 回到 root| Root
    T5 -->|匹配 '国'?| 国
```

#### DFA 算法敏感词匹配流程动画版特点：
1. **没有 fail 指针**，匹配失败必须回到 root 重新扫描。
2. **文本扫描箭头**表示每个字符与 DFA 树节点的匹配尝试。
3. **output 节点**表示匹配成功的敏感词。
4. 可以直观看到 DFA 匹配效率低于 AC，因为匹配失败要回到 root。
